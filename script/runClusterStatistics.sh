...
bsub -M ${MEMORY_LIMIT} -R "rusage[mem=${MEMORY_LIMIT}]" -q production-rh6 -g /pride_cluster_loader -u rwang@ebi.ac.uk -J ${JOB_NAME} java -Xmx${MEMORY_LIMIT}m -jar ${EXE_JAR} ${CLUSTER_FILE_DIRECTORY} ${OVERALL_STAT_OUTPUT_FILE} ${CLUSTER_STAT_OUTPUT_FILE}
